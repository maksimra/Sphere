...
void Sphere::get_sphere_pixels (unsigned char* pixels, const Lighting& lighting, const GraphicSystem& graphic_system,
    ...
    for (size_t y_pixel_number = 0; y_pixel_number < graphic_system.get_y_size (); y_pixel_number++)
    {
        for (size_t x_pixel_number = 0; x_pixel_number < graphic_system.get_x_size (); x_pixel_number++)
        {
                ...
                square (radius_))
            {
                ...
                memcpy (pixels + 4 * (x_pixel_number + y_pixel_number * graphic_system.get_x_size ()),
                        &pixel_color,
                        sizeof (int));
...
void set_color (unsigned int* color, const Vector3& light_color, double ambient_coeff, const Vector3& sphere_color,
                ...
                int point_sphere_x, int point_sphere_y)
{
    ...
    Vector3 rgb_vector = multiple_of_elements (sphere_color, light_color * (cos_between + ambient_coeff));
    fill_color (color, rgb_vector);
    ...
    sphere_color = sphere_color & ALPHA_CHANNEL_MASK;
...
//
    //*color = sphere_color | red_channel | green_channel | blue_channel;
    //printf ("color == 0x%x\n", *color);
}

void fill_color (unsigned int* color, const Vector3& rgb_vector)
{
    unsigned int red_channel = 0;
    unsigned int green_channel = 0;
    unsigned int blue_channel = 0;
    if ((red_channel = rgb_vector.get_x ()) <= 0xFF)
        red_channel = red_channel << RED_SHIFT;
    else
        red_channel = 0xFF;

    if ((green_channel = rgb_vector.get_y ()) <= 0xFF)
        green_channel = green_channel << GREEN_SHIFT;
    else
        green_channel = 0xFF00;

    if ((blue_channel = blue_vector.get_z ()) <= 0xFF)
        blue_channel = blue_channel >> BLUE_SHIFT;
    else
        blue_channel = 0xFF00;

    *color = 0xAA000000 | red_channel | blue_channel | green_channel; // спросить насчёт прозрачности
}
